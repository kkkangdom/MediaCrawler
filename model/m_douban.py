...
class DoubanPost(BaseModel):
    """
    Douban post/discussion model
    """
    ...
    content_url: str = Field(default="", description="Post URL")
    ...
    group_id: str = Field(default="", description="Group/Topic ID")
    
    # Media URLs
    image_urls: List[str] = Field(default_factory=list, description="Image URLs")
    

class DoubanComment(BaseModel):
    """
    Douban comment model
    """
    ...
    post_id: str = Field(default="", description="Post ID this comment belongs to")
    like_count: int = Field(default=0, description="Like count")
